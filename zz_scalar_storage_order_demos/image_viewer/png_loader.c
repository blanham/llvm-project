...
// Supports: IHDR, IDAT (zlib inflate via system zlib), PLTE (palette), tRNS (palette / grayscale transparency), no interlace, 8-bit modes: RGBA (6), RGB (2), Gray (0), Indexed (3), Gray+Alpha (4).
// CRC is verified (warnings on mismatch). Ancillary chunks logged & skipped. Adam7 interlace not implemented.
...
static int load_png_stream(FILE *f, int manual, int decode_pixels, ImageData *out){
    ...
    uint32_t crc_calc = c ^ 0xFFFFFFFFu;
        ...
        if(!strcmp(type,"IHDR")){
            ...
            if(interlace){ fprintf(stderr,"PNG interlace not supported (Adam7)\n"); }
    ...
    size_t rowBytes = 1 + w * bpp_raw; // filter byte + raw pixels (8-bit depth only)
    ...
    for(uint32_t y=0;y<h;y++){
        ...
        for(uint32_t x=0;x<w;x++){
            ...
            if(color_type==2){ size_t srci = x*3; pixels[di+0]=px[srci+0]; pixels[di+1]=px[srci+1]; pixels[di+2]=px[srci+2]; pixels[di+3]=255; }
            else if(color_type==6){ size_t srci = x*4; pixels[di+0]=px[srci+0]; pixels[di+1]=px[srci+1]; pixels[di+2]=px[srci+2]; pixels[di+3]=px[srci+3]; }
                ...
                unsigned idx=px[x]; if(idx>=palette_entries){ pixels[di+0]=pixels[di+1]=pixels[di+2]=0; pixels[di+3]=255; }
                else { pixels[di+0]=palette[idx*3+0]; pixels[di+1]=palette[idx*3+1]; pixels[di+2]=palette[idx*3+2]; pixels[di+3] = (trns && idx<trns_len)? trns[idx]:255; }
